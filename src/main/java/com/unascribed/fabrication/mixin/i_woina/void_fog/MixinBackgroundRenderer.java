...
public abstract class MixinBackgroundRenderer {
	...
	private static void fabrication$voidFogColor(Camera camera, float tickDelta, ClientWorld world, int i, float f, CallbackInfo ci) {
		...
		float dist = Math.abs((float) entity.getY());
	...
	private static void fabrication$voidFogDist(Camera camera, BackgroundRenderer.FogType fogType, float viewDistance, boolean thickFog, CallbackInfo ci) {
		if (!FabConf.isEnabled("*.void_fog")) return;
