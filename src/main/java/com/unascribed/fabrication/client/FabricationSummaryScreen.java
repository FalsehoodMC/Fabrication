...
public class FabricationSummaryScreen extends Screen {
	...
	public FabricationSummaryScreen(Screen parent, List<String> options) {
		super(Text.of(MixinConfigPlugin.MOD_NAME + " summary"));
	...
	public void render(DrawContext drawContext, int mouseX, int mouseY, float delta) {
		FabricationConfigScreen.drawBackground(height, width, client, prideFlag, 0, drawContext, 0, 0, 0, 0, 0);
		...
		for (String key : features) {
			...
			float h = 0;
			if (entry == null) {
				h = drawWrappedText(drawContext, 25, y, "Unknown feature: "+key, width - 100, 0xFFFFFF, false) + 6;
				scrollBar.height += h;
				y+=h;
			} else {
				h = drawWrappedText(drawContext, 25, y, entry.name, width - 100, 0xFFFFFF, false) + 6;
				h += drawWrappedText(drawContext, 50, y+h, entry.desc, width - 100, 0xFFFFFF, false) + 6;
				scrollBar.height += h;
				y+=h;
			}
		}
