...
public class FeatureSwapConflictingEnchants implements Feature {
	...
	private void applyClient() {
		Agnos.runForTooltipRender((stack, lines) -> {
			if (!stack.isEmpty() && stack.hasTag() && stack.getTag().contains("fabrication#conflictingEnchants")) {
				if ((FabRefl.ItemStack_getHideFlags(stack) & ItemStack.TooltipSection.ENCHANTMENTS.getFlag()) != 0) return;
				NbtCompound lTag = stack.getTag().getCompound("fabrication#conflictingEnchants");
				if (lTag == null || lTag.isEmpty()) return;
				int ii=0;
				for (int i=0;i<lines.size();i++) {
					Text o = lines.get(i);
					if (o instanceof TranslatableText) {
						String ke = ((TranslatableText) o).getKey();
				...
				for (String key : lTag.getKeys()) {
					Optional<Enchantment> e = Registry.ENCHANTMENT.getOrEmpty(Identifier.tryParse(key));
					if (e.isPresent()) {
