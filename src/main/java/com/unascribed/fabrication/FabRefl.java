...
public class FabRefl {
	...
	private static final MethodHandle cpc2sp_channel = unreflectGetter(CustomPayloadC2SPacket.class, "channel", "field_12830", "field_149562_a").get();
	...
	private static final MethodHandle cpc2sp_data = unreflectGetter(CustomPayloadC2SPacket.class, "data", "field_12832", "field_149561_c").get();
	...
	private static UnreflResult unreflectGetter(Class<?> clazz, String yarnName, String interName, String srgName) {
		String name = DEV ? yarnName : FORGE ? srgName : interName;
		String desc = "field "+clazz.getName()+"#"+name+" (deobf name "+yarnName+")";
		try {
	...
	private static UnreflResult unreflectSetter(Class<?> clazz, String yarnName, String interName, String srgName) {
		String name = DEV ? yarnName : FORGE ? srgName : interName;
		String desc = "field "+clazz.getName()+"#"+name+" (deobf name "+yarnName+")";
		try {
			Field f = clazz.getDeclaredField(name);
	...
	private static UnreflResult unreflectMethod(Class<?> clazz, String yarnName, String interName, String srgName, Class<?> returnType, Class<?>... args) {
		String name = DEV ? yarnName : FORGE ? srgName : interName;
		String desc = "method "+clazz.getName()+"."+name+signatureToString(args)+" (deobf name "+yarnName+")";
		try {
	...
	private static UnreflResult unreflectConstructor(Class<?> clazz, Class<?>... args) {
		String desc = "constructor "+clazz.getName()+signatureToString(args);
		try {
